...
FORBIDDEN_CODE_PATTERNS = {
    ...
    "pdMS_TO_TICKS": re.compile(r"\bpdMS_TO_TICKS\s*\("),
    ...
    "ESP_EARLY_LOG": re.compile(r"\bESP_EARLY_LOG[A-Z]*\s*\("),
    "ESP_DRAM_LOG": re.compile(r"\bESP_DRAM_LOG[A-Z]*\s*\("),
    ...
    "std::chrono": re.compile(r"\bstd::chrono\b"),
    "std::this_thread": re.compile(r"\bstd::this_thread\b"),
...
FORBIDDEN_RAW_PATTERNS = {
    ...
    "freertos token": re.compile(r"\bfreertos\b"),
    "ESP-IDF token": re.compile(r"\bESP-IDF\b"),
    "ESP32 token": re.compile(r"\bESP32\b"),
    "esp_err_t token": re.compile(r"\besp_err_t\b"),
    "esp_ token": re.compile(r"\besp_[A-Za-z0-9_]*\b"),
    "ESP_ token": re.compile(r"\bESP_[A-Za-z0-9_]*\b"),
    "i2c_master token": re.compile(r"\bi2c_master\b"),
    "gpio_num_t token": re.compile(r"\bgpio_num_t\b"),
    "TaskHandle_t token": re.compile(r"\bTaskHandle_t\b"),
    "SemaphoreHandle_t token": re.compile(r"\bSemaphoreHandle_t\b"),
    "TickType_t token": re.compile(r"\bTickType_t\b"),
    "ARDUINO token": re.compile(r"\bARDUINO\b"),
...
FORBIDDEN_INCLUDE_RE = re.compile(
    ...
    r'driver/i2c_master\.h|esp_timer\.h|esp_log\.h|esp_err\.h|esp_check\.h|'
    r'esp_system\.h|sdkconfig\.h|esp_rom_sys\.h|rom/ets_sys\.h|soc/[^>"]+|'
    r'hal/[^>"]+|unistd\.h|sys/time\.h|time\.h|chrono|thread|iostream)'
    r'[>"]',
    re.MULTILINE,
)
