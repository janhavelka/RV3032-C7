...
struct NativeTestAccess {
  using JobState = RV3032::JobState;
  using JobKind = RV3032::JobKind;
  using EepromState = RV3032::EepromState;
  static auto& job(RV3032& rtc) { return rtc._job; }
  static auto& eeprom(RV3032& rtc) { return rtc._eeprom; }
  static uint8_t noWaitCallbackCap(JobKind kind) {
    return RV3032::NO_WAIT_JOB_CALLBACK_CAPS[static_cast<size_t>(kind)];
  }
  static bool unsupportedRange(uint8_t reg, size_t len) {
    return RV3032::intersectsUnsupportedPasswordRange(reg, len);
  }
};
}  // namespace RV3032
...
static_assert(static_cast<uint8_t>(RV3032::Err::OK) == 0);
static_assert(static_cast<uint8_t>(RV3032::Err::NOT_INITIALIZED) == 1);
static_assert(static_cast<uint8_t>(RV3032::Err::INVALID_CONFIG) == 2);
static_assert(static_cast<uint8_t>(RV3032::Err::I2C_ERROR) == 3);
static_assert(static_cast<uint8_t>(RV3032::Err::TIMEOUT) == 4);
static_assert(static_cast<uint8_t>(RV3032::Err::INVALID_PARAM) == 5);
static_assert(static_cast<uint8_t>(RV3032::Err::INVALID_DATETIME) == 6);
static_assert(static_cast<uint8_t>(RV3032::Err::DEVICE_NOT_FOUND) == 7);
static_assert(static_cast<uint8_t>(RV3032::Err::EEPROM_WRITE_FAILED) == 8);
static_assert(static_cast<uint8_t>(RV3032::Err::REGISTER_READ_FAILED) == 9);
static_assert(static_cast<uint8_t>(RV3032::Err::REGISTER_WRITE_FAILED) == 10);
static_assert(static_cast<uint8_t>(RV3032::Err::QUEUE_FULL) == 11);
static_assert(static_cast<uint8_t>(RV3032::Err::BUSY) == 12);
static_assert(static_cast<uint8_t>(RV3032::Err::IN_PROGRESS) == 13);
static_assert(static_cast<uint8_t>(RV3032::Err::I2C_NACK_ADDR) == 14);
static_assert(static_cast<uint8_t>(RV3032::Err::I2C_NACK_DATA) == 15);
static_assert(static_cast<uint8_t>(RV3032::Err::I2C_TIMEOUT) == 16);
static_assert(static_cast<uint8_t>(RV3032::Err::I2C_BUS) == 17);
static_assert(static_cast<uint8_t>(RV3032::Err::EEPROM_VERIFY_FAILED) == 18);
static_assert(static_cast<uint8_t>(RV3032::Err::EEPROM_CLEANUP_FAILED) == 19);
static_assert(
    static_cast<uint8_t>(RV3032::Err::PRIMARY_CELL_ALREADY_ATTEMPTED) == 20);
static_assert(static_cast<uint8_t>(RV3032::Err::JOB_RESULT_UNAVAILABLE) == 21);
static_assert(static_cast<uint8_t>(RV3032::Err::INCOHERENT_DATA) == 22);
static_assert(
    static_cast<uint8_t>(RV3032::Err::CONFIGURATION_CLEANUP_FAILED) == 23);
static_assert(
    static_cast<uint8_t>(RV3032::Err::TRANSPORT_CONTRACT_VIOLATION) == 24);
static_assert(static_cast<uint8_t>(RV3032::Err::INTERNAL_STATE_ERROR) == 25);
...
RV3032::Status pollJobToCompletion(RV3032::RV3032& rtc, FakeRv3032& fake,
  ...
  RV3032::Status st = RV3032::Status::Error(RV3032::Err::IN_PROGRESS,
                                             "not started");
  const uint8_t callbackCap = RV3032::NativeTestAccess::noWaitCallbackCap(
      RV3032::NativeTestAccess::job(rtc).activeKind);
  const uint32_t callbacksAtStart = fake.callbackCount;
  for (uint16_t i = 0; i < callCap; ++i) {
    uint8_t used = 0;
    const uint32_t callbacksBeforePoll = fake.callbackCount;
    st = rtc.pollJob(fake.nowMs, budget, used);
    if (callbackCap != 0) {
      TEST_ASSERT_LESS_OR_EQUAL_UINT32(callbackCap,
          fake.callbackCount - callbacksAtStart);
    }
...
void assertPrimaryFaultCleanup(const FakeRv3032& fake,
  ...
  TEST_ASSERT_FALSE(fake.passwordCommandAttempted);
...
void test_config_validation_is_zero_io_and_non_destructive() {
  ...
  cfg = fake.config();
  cfg.primaryCellI2cTimeoutMs = 0;
  st = rtc.begin(cfg);
  TEST_ASSERT_EQUAL_UINT8(static_cast<uint8_t>(RV3032::Err::INVALID_CONFIG),
                          static_cast<uint8_t>(st.code));
  cfg = fake.config();
  cfg.primaryCellI2cTimeoutMs = 6;
  st = rtc.begin(cfg);
  TEST_ASSERT_EQUAL_UINT8(static_cast<uint8_t>(RV3032::Err::INVALID_CONFIG),
                          static_cast<uint8_t>(st.code));
  ...
  cfg.nowMs = nullptr;
  cfg.i2cTimeoutMs = 1;
  for (uint32_t invalidTimeout : {0U, 9U, 251U, UINT32_MAX}) {
    cfg.eepromTimeoutMs = invalidTimeout;
    TEST_ASSERT_EQUAL_INT(static_cast<int>(RV3032::Err::INVALID_CONFIG),
                          static_cast<int>(rtc.begin(cfg).code));
    TEST_ASSERT_FALSE(rtc.isInitialized());
    TEST_ASSERT_EQUAL_UINT32(0, fake.callbackCount);
  }
...
void test_poll_eeprom_zero_budget_tick_and_end_guards() {
  FakeRv3032 fake;
  FakeRv3032 rebound;
  RV3032::RV3032 rtc;
  ...
  used = 99;
  const uint32_t beforeZeroBudget = fake.callbackCount;
  TEST_ASSERT_TRUE(rtc.pollEeprom(fake.nowMs, 0, used).inProgress());
  TEST_ASSERT_EQUAL_UINT8(0, used);
  TEST_ASSERT_EQUAL_UINT8(1, rtc.eepromQueueDepth());
  TEST_ASSERT_EQUAL_UINT32(beforeZeroBudget, fake.callbackCount);
  ...
  used = 99;
  TEST_ASSERT_TRUE(rtc.pollEeprom(rebound.nowMs, 0, used).ok());
  TEST_ASSERT_EQUAL_UINT8(0, used);
...
void test_synchronous_writes_reject_pending_cooperative_work() {
  FakeRv3032 fake;
  fake.setCalendar(2026, 7, 13, 12, 34, 56, 1);
  fake.direct[RV3032::cmd::REG_STATUS] = 0;
  RV3032::RV3032 rtc;
  TEST_ASSERT_TRUE(rtc.begin(fake.config()).ok());
  const RV3032::DateTime requested{2026, 7, 13, 12, 35, 0, 1};
  TEST_ASSERT_TRUE(rtc.startSetTimeAndClearInvalidFlagsVerifiedJob(
      requested, fake.nowMs, 250).inProgress());

  const uint8_t small[2] = {0xA5, 0x5A};
  uint8_t full[16] = {};
  const uint32_t callbacksBefore = fake.callbackCount;
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::Err::BUSY),
      static_cast<uint8_t>(rtc.setTime(requested).code));
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::Err::BUSY),
      static_cast<uint8_t>(rtc.setUnix(946684800UL).code));
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::Err::BUSY),
      static_cast<uint8_t>(rtc.writeRegister(
          RV3032::cmd::REG_USER_RAM_START, small[0]).code));
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::Err::BUSY),
      static_cast<uint8_t>(rtc.writeRegisters(
          RV3032::cmd::REG_USER_RAM_START, small, sizeof(small)).code));
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::Err::BUSY),
      static_cast<uint8_t>(rtc.writeUserRam(0, small, sizeof(small)).code));
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::Err::BUSY),
      static_cast<uint8_t>(rtc.writeUserRam(0, full, sizeof(full)).code));
  TEST_ASSERT_EQUAL_UINT32(callbacksBefore, fake.callbackCount);

  RV3032::DateTime observed{};
  uint8_t hundredths = 0xFF;
  TEST_ASSERT_TRUE(rtc.readTime(observed).ok());
  TEST_ASSERT_TRUE(rtc.readHundredths(hundredths).ok());
  TEST_ASSERT_EQUAL_UINT8(0, hundredths);
  TEST_ASSERT_TRUE(pollJobToCompletion(rtc, fake).ok());

  FakeRv3032 persistenceFake;
  persistenceFake.setCalendar(2026, 7, 13, 12, 34, 56, 1);
  RV3032::RV3032 persistenceRtc;
  TEST_ASSERT_TRUE(persistenceRtc.begin(persistenceFake.config(true)).ok());
  TEST_ASSERT_TRUE(persistenceRtc.setOffsetPpm(0.238418579f).inProgress());
  TEST_ASSERT_TRUE(
      pollJobToCompletion(persistenceRtc, persistenceFake, 1).ok());
  TEST_ASSERT_TRUE(persistenceRtc.isEepromBusy());
  const uint32_t beforeQueuedWrites = persistenceFake.callbackCount;
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::Err::BUSY),
      static_cast<uint8_t>(persistenceRtc.setTime(requested).code));
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::Err::BUSY),
      static_cast<uint8_t>(persistenceRtc.writeRegister(
          RV3032::cmd::REG_USER_RAM_START, small[0]).code));
  TEST_ASSERT_EQUAL_UINT32(beforeQueuedWrites,
                           persistenceFake.callbackCount);
  TEST_ASSERT_TRUE(persistenceRtc.readTime(observed).ok());

  uint8_t eepromUsed = 0;
  TEST_ASSERT_TRUE(persistenceRtc.pollEeprom(
      persistenceFake.nowMs, 1, eepromUsed).inProgress());
  const uint32_t beforeActiveWrites = persistenceFake.callbackCount;
  TEST_ASSERT_TRUE(persistenceRtc.isJobBusy());
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::Err::BUSY),
      static_cast<uint8_t>(persistenceRtc.setTime(requested).code));
  TEST_ASSERT_EQUAL_UINT32(beforeActiveWrites,
                           persistenceFake.callbackCount);
  TEST_ASSERT_TRUE(
      pollEepromToCompletion(persistenceRtc, persistenceFake, 4).ok());
}
...
void test_verified_calendar_rejects_mismatched_readback_weekday() {
  FakeRv3032 fake;
  fake.direct[RV3032::cmd::REG_STATUS] = 0;
  RV3032::RV3032 rtc;
  TEST_ASSERT_TRUE(rtc.begin(fake.config()).ok());
  ...
  const RV3032::Status terminal = pollJobToCompletion(rtc, fake);
  TEST_ASSERT_EQUAL_UINT8(
...
void test_impossible_ordinary_states_fail_without_transport() {
  using Access = RV3032::NativeTestAccess;
  for (auto state : {Access::JobState::IDLE,
                     static_cast<Access::JobState>(255)}) {
    FakeRv3032 fake;
    RV3032::RV3032 rtc;
    TEST_ASSERT_TRUE(rtc.begin(fake.config()).ok());
    TEST_ASSERT_TRUE(rtc.setClockInterruptEnabled(true).inProgress());
    Access::job(rtc).state = state;
    uint8_t used = 99;
    TEST_ASSERT_EQUAL_INT(static_cast<int>(RV3032::Err::INTERNAL_STATE_ERROR),
        static_cast<int>(rtc.pollJob(fake.nowMs, 1, used).code));
    TEST_ASSERT_EQUAL_UINT8(0, used);
    TEST_ASSERT_EQUAL_UINT32(0, fake.callbackCount);
  ...
  Access::job(rtc).state = Access::JobState::BACKUP_VERIFY_PMU;
  TEST_ASSERT_EQUAL_INT(static_cast<int>(RV3032::Err::INTERNAL_STATE_ERROR),
      static_cast<int>(pollJobToCompletion(rtc, fake).code));
  RV3032::ConfigurationJobReport report{};
  TEST_ASSERT_EQUAL_INT(static_cast<int>(RV3032::Err::INTERNAL_STATE_ERROR),
      static_cast<int>(rtc.getSetBackupSwitchModeJobResult(report).code));
  TEST_ASSERT_FALSE(report.mutationAttempted);
  TEST_ASSERT_EQUAL_UINT32(0, fake.callbackCount);
}

void test_impossible_persistent_states_preserve_cleanup_evidence() {
  using Access = RV3032::NativeTestAccess;
  for (bool missingControl1 : {false, true}) {
    FakeRv3032 fake;
    RV3032::RV3032 rtc;
    TEST_ASSERT_TRUE(rtc.begin(fake.config()).ok());
    TEST_ASSERT_TRUE(rtc.startReadUserEepromJob(0, 1, fake.nowMs).inProgress());
    auto& op = Access::job(rtc).persistent;
    op.state = missingControl1 ? Access::EepromState::RESTORE_CONTROL1
                              : static_cast<Access::EepromState>(255);
    op.cleanupRequired = missingControl1;
    const RV3032::Err expected = missingControl1
        ? RV3032::Err::EEPROM_CLEANUP_FAILED : RV3032::Err::INTERNAL_STATE_ERROR;
    TEST_ASSERT_EQUAL_INT(static_cast<int>(expected),
        static_cast<int>(pollJobToCompletion(rtc, fake).code));
    RV3032::PersistentReadResult report{};
    TEST_ASSERT_EQUAL_INT(static_cast<int>(expected),
        static_cast<int>(rtc.getPersistentReadJobResult(report).code));
    const RV3032::Status cause = missingControl1
        ? report.cleanupStatus : report.operationStatus;
    TEST_ASSERT_EQUAL_INT(static_cast<int>(RV3032::Err::INTERNAL_STATE_ERROR),
                          static_cast<int>(cause.code));
    TEST_ASSERT_FALSE(report.cleanupVerified);
    TEST_ASSERT_EQUAL(missingControl1,
                      rtc.getSettings().persistentAccessStateUnproven);
    TEST_ASSERT_EQUAL_UINT32(0, fake.callbackCount);
  }

  // Corrupt an active access job after real EERD/safe-C0 staging: its saved
  // restoration evidence must still drive bounded cleanup, with no EECMD.
  FakeRv3032 fake;
  RV3032::RV3032 rtc;
  TEST_ASSERT_TRUE(rtc.begin(fake.config()).ok());
  TEST_ASSERT_TRUE(rtc.startReadUserEepromJob(0, 1, fake.nowMs).inProgress());
  uint8_t used = 0;
  TEST_ASSERT_TRUE(rtc.pollJob(fake.nowMs, 7, used).inProgress());
  TEST_ASSERT_TRUE(Access::job(rtc).persistent.safeC0Verified);
  Access::job(rtc).persistent.state = static_cast<Access::EepromState>(255);
  TEST_ASSERT_EQUAL_INT(static_cast<int>(RV3032::Err::INTERNAL_STATE_ERROR),
      static_cast<int>(pollJobToCompletion(rtc, fake).code));
  RV3032::PersistentReadResult report{};
  TEST_ASSERT_EQUAL_INT(static_cast<int>(RV3032::Err::INTERNAL_STATE_ERROR),
...
void test_password_range_helper_rejects_invalid_spans() {
  using Access = RV3032::NativeTestAccess;
  TEST_ASSERT_TRUE(Access::unsupportedRange(0, 0));
  TEST_ASSERT_TRUE(Access::unsupportedRange(255, 2));
  TEST_ASSERT_TRUE(Access::unsupportedRange(0, SIZE_MAX));
  TEST_ASSERT_FALSE(Access::unsupportedRange(0, 57));
}

void test_default_temperature_job_executes_without_clock_hook() {
  FakeRv3032 fake;
  RV3032::Config config = fake.config();
  config.nowMs = nullptr;
  ...
  RV3032::RV3032 rtc;
  TEST_ASSERT_TRUE(rtc.begin(config).ok());
  TEST_ASSERT_TRUE(rtc.startReadCoherentTemperatureJob(0).inProgress());
  TEST_ASSERT_EQUAL_UINT32(0, fake.callbackCount);
  uint8_t used = 0;
  TEST_ASSERT_TRUE(rtc.pollJob(0, 2, used).ok());
  TEST_ASSERT_EQUAL_UINT8(2, used);
  RV3032::CoherentTemperatureResult result{};
  TEST_ASSERT_TRUE(rtc.getReadCoherentTemperatureJobResult(result).ok());
}

void test_default_backup_job_executes_at_large_callback_timeouts() {
  for (uint32_t timeoutMs : {60U, 100U}) {
    for (bool clockHook : {false, true}) {
      FakeRv3032 fake;
      fake.activeConfig[0] = 0;
      fake.callbackDurationMs = timeoutMs;
      RV3032::Config config = fake.config();
      config.i2cTimeoutMs = timeoutMs;
      if (!clockHook) config.nowMs = nullptr;
      RV3032::RV3032 rtc;
      TEST_ASSERT_TRUE(rtc.begin(config).ok());
      TEST_ASSERT_TRUE(rtc.startSetBackupSwitchModeJob(
          RV3032::BackupSwitchMode::Level, fake.nowMs).inProgress());
      ...
      TEST_ASSERT_EQUAL_INT(static_cast<int>(
          RV3032::ConfigurationFinalState::REQUESTED_VERIFIED),
          static_cast<int>(report.finalState));
    }
  }
}

void test_default_calendar_jobs_admit_without_clock_hook() {
  FakeRv3032 snapshotFake;
  RV3032::Config snapshotConfig = snapshotFake.config();
  snapshotConfig.nowMs = nullptr;
  snapshotConfig.i2cTimeoutMs = 50;
  RV3032::RV3032 snapshotRtc;
  TEST_ASSERT_TRUE(snapshotRtc.begin(snapshotConfig).ok());
  TEST_ASSERT_TRUE(snapshotRtc.startReadTimeSnapshotJob(0).inProgress());
  TEST_ASSERT_EQUAL_UINT32(0, snapshotFake.callbackCount);
  snapshotRtc.end();

  FakeRv3032 setFake;
  RV3032::Config setConfig = setFake.config();
  setConfig.nowMs = nullptr;
  setConfig.i2cTimeoutMs = 50;
  RV3032::RV3032 setRtc;
  TEST_ASSERT_TRUE(setRtc.begin(setConfig).ok());
  const RV3032::DateTime requested{2026, 7, 13, 12, 0, 0, 1};
  TEST_ASSERT_TRUE(setRtc.startSetTimeAndClearInvalidFlagsVerifiedJob(
      requested, 0).inProgress());
  TEST_ASSERT_EQUAL_UINT32(0, setFake.callbackCount);
  setRtc.end();

  FakeRv3032 maxSnapshotFake;
  RV3032::Config maxSnapshotConfig = maxSnapshotFake.config();
  maxSnapshotConfig.nowMs = nullptr;
  maxSnapshotConfig.i2cTimeoutMs = 100;
  RV3032::RV3032 maxSnapshotRtc;
  TEST_ASSERT_TRUE(maxSnapshotRtc.begin(maxSnapshotConfig).ok());
  TEST_ASSERT_TRUE(
      maxSnapshotRtc.startReadTimeSnapshotJob(0).inProgress());
  uint8_t used = 0;
  TEST_ASSERT_TRUE(maxSnapshotRtc.pollJob(0, 2, used).ok());
  TEST_ASSERT_GREATER_THAN_UINT8(0, used);
  TEST_ASSERT_LESS_OR_EQUAL_UINT8(2, used);

  FakeRv3032 maxSetFake;
  maxSetFake.direct[RV3032::cmd::REG_STATUS] = 0;
  RV3032::Config maxSetConfig = maxSetFake.config();
  maxSetConfig.nowMs = nullptr;
  maxSetConfig.i2cTimeoutMs = 100;
  RV3032::RV3032 maxSetRtc;
  TEST_ASSERT_TRUE(maxSetRtc.begin(maxSetConfig).ok());
  TEST_ASSERT_TRUE(maxSetRtc.startSetTimeAndClearInvalidFlagsVerifiedJob(
      requested, 0).inProgress());
  TEST_ASSERT_TRUE(maxSetRtc.pollJob(0, 7, used).ok());
  TEST_ASSERT_GREATER_THAN_UINT8(0, used);
  TEST_ASSERT_LESS_OR_EQUAL_UINT8(7, used);
}
...
void test_raw_access_allowlists_block_side_effect_routes() {
  ...
  TEST_ASSERT_EQUAL_UINT8(
      ...
      static_cast<uint8_t>(rtc.writeRegisters(
          RV3032::cmd::REG_ACTIVE_TREFERENCE1, crossing, 2).code));
  const size_t oversized = static_cast<size_t>(-1);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::Err::INVALID_PARAM),
      static_cast<uint8_t>(rtc.readRegisters(2, crossing, oversized).code));
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::Err::INVALID_PARAM),
      static_cast<uint8_t>(rtc.writeRegisters(2, crossing, oversized).code));
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::Err::INVALID_PARAM),
      static_cast<uint8_t>(rtc.readRegisters(0xFF, crossing, 2).code));
  TEST_ASSERT_EQUAL_UINT32(before, fake.callbackCount);
...
void test_persistent_dynamic_cleanup_reserve_admission_is_zero_io() {
  ...
  const uint32_t eepromTimeouts[] = {10, 100, 250};
  for (uint32_t callbackTimeout : callbackTimeouts) {
    for (uint32_t eepromTimeout : eepromTimeouts) {
      FakeRv3032 fake;
      RV3032::Config config = fake.config(true);
      config.i2cTimeoutMs = callbackTimeout;
      config.eepromTimeoutMs = eepromTimeout;
      RV3032::RV3032 rtc;
      TEST_ASSERT_TRUE(rtc.begin(config).ok());
      const uint32_t finalCleanupReserve =
          250U + 6U * callbackTimeout + 10U;
      const uint32_t readMinimum =
          finalCleanupReserve + callbackTimeout + 3U;
      const uint32_t postWriteReserve =
          10U + eepromTimeout + 12U * callbackTimeout +
          2U * 25U + finalCleanupReserve;
      const uint32_t writeMinimum =
          postWriteReserve + callbackTimeout + 3U;
      const uint32_t before = fake.callbackCount;
      TEST_ASSERT_EQUAL_UINT8(
          static_cast<uint8_t>(RV3032::Err::INVALID_PARAM),
          static_cast<uint8_t>(rtc.startReadConfigurationEepromJob(
              RV3032::ConfigurationEepromRegister::PMU,
              fake.nowMs, readMinimum - 1U).code));
      const uint8_t value = 0xA5;
      TEST_ASSERT_EQUAL_UINT8(
          static_cast<uint8_t>(RV3032::Err::INVALID_PARAM),
          static_cast<uint8_t>(rtc.startWriteUserEepromJob(
              0, &value, 1, fake.nowMs, writeMinimum - 1U).code));
      TEST_ASSERT_EQUAL_UINT32(before, fake.callbackCount);
      TEST_ASSERT_TRUE(rtc.startWriteUserEepromJob(
          0, &value, 1, fake.nowMs, writeMinimum).inProgress());
      TEST_ASSERT_EQUAL_UINT32(before, fake.callbackCount);
      rtc.end();

      RV3032::RV3032 readRtc;
      TEST_ASSERT_TRUE(readRtc.begin(config).ok());
      TEST_ASSERT_TRUE(readRtc.startReadConfigurationEepromJob(
          RV3032::ConfigurationEepromRegister::PMU,
          fake.nowMs, readMinimum).inProgress());
      TEST_ASSERT_EQUAL_UINT32(before, fake.callbackCount);
      readRtc.end();
    }
  }
...
void test_settings_snapshot_tracks_job_queue_health_and_deadlines() {
  ...
  config.primaryCellI2cTimeoutMs = 4;
  ...
  TEST_ASSERT_EQUAL_UINT32(4, snapshot.primaryCellI2cTimeoutMs);
...
void test_fixture_wait_request_log_is_bounded_and_reports_overflow() {
  FakeRv3032 fake;
  fake.earlyWait = true;
...
void test_generic_persistence_honors_extended_busy_poll_window() {
  FakeRv3032 fake;
  RV3032::Config config = fake.config(true);
  config.eepromTimeoutMs = 250;
  RV3032::RV3032 rtc;
  TEST_ASSERT_TRUE(rtc.begin(config).ok());
  ...
  uint8_t used = 0;
  for (uint16_t poll = 0; poll < 1000 && fake.writeOneAttempts == 0; ++poll) {
    TEST_ASSERT_TRUE(rtc.pollEeprom(fake.nowMs, 1, used).inProgress());
    if (used == 0) ++fake.nowMs;
  }
  TEST_ASSERT_EQUAL_UINT16(1, fake.writeOneAttempts);
  const uint32_t commandTime = fake.nowMs;
  fake.setInitialBusy(150);
  TEST_ASSERT_TRUE(pollEepromToCompletion(rtc, fake, 1).ok());
  TEST_ASSERT_GREATER_OR_EQUAL_UINT32(commandTime + 150U, fake.nowMs);
  TEST_ASSERT_GREATER_THAN_UINT32(101, fake.callbackCount);
  TEST_ASSERT_EQUAL_UINT32(1, rtc.eepromWriteCount());
}
...
void test_generic_multi_instruction_poll_refreshes_mutation_cutoff() {
  ...
  TEST_ASSERT_EQUAL_UINT16(0, fake.writeOneAttempts);
  // The post-write reserve is 510 ms at this fixture's defaults, so the
  // generic 4000 ms item cutoff is 3490 ms.
  fake.nowMs = 3488;
...
void test_primary_cell_callback_timeouts_clip_to_transfer_and_overall() {
  {
    ...
    RV3032::Config config = fake.config();
    config.i2cTimeoutMs = 100;
    config.primaryCellI2cTimeoutMs = 3;
    RV3032::RV3032 rtc;
    TEST_ASSERT_TRUE(rtc.begin(config).ok());
    RV3032::PrimaryCellConfigurationReport report{};
    TEST_ASSERT_TRUE(rtc.ensurePrimaryCellConfiguration(report).ok());
    ...
    for (size_t i = 0; i < fake.logCount; ++i) {
      ...
      TEST_ASSERT_LESS_OR_EQUAL_UINT32(3, fake.log[i].timeoutMs);
      sawTransferCap = sawTransferCap || fake.log[i].timeoutMs == 3;
    }
...
void test_primary_cell_acked_ignore_and_low_vdd_fail_safely() {
...

  {
    ...
    TEST_ASSERT_EQUAL_UINT8(
        static_cast<uint8_t>(
            RV3032::PrimaryCellFailureStage::VERIFY_PERSISTENT),
...
void test_static_calendar_and_status_utility_coverage() {
  ...
  const RV3032::Status outsideDomain = RV3032::RV3032::computeWeekday(
      2100, 1, 1, unchangedWeekday);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::Err::INVALID_DATETIME),
      static_cast<uint8_t>(outsideDomain.code));
  TEST_ASSERT_EQUAL_STRING(
      "Date outside the supported 2000..2099 domain", outsideDomain.msg);
  TEST_ASSERT_EQUAL_HEX8(0xA5, unchangedWeekday);
  const RV3032::Status invalidGregorian = RV3032::RV3032::computeWeekday(
      2026, 2, 30, unchangedWeekday);
  TEST_ASSERT_EQUAL_STRING("Invalid Gregorian date", invalidGregorian.msg);
...
void test_alarm_timer_and_pmu_round_trips_are_cooperative() {
  ...
  fake.direct[RV3032::cmd::REG_ALARM_DATE] = 0x80;
  TEST_ASSERT_TRUE(rtc.getAlarmConfig(alarm).ok());
  TEST_ASSERT_EQUAL_UINT8(0, alarm.date);
...
void test_fake_refresh_initial_busy_and_acked_ignore_faults() {
  ...
  TEST_ASSERT_FALSE(busyFake.passwordCommandAttempted);
...
void test_queue_cleanup_failure_cancels_later_items() {
  ...
  uint32_t cleanupControl1WriteOrdinal = 0;
  ...
  for (size_t i = 0; i < probe.logCount; ++i) {
    if (probe.log[i].write &&
      ...
      cleanupPmuWriteOrdinal = static_cast<uint32_t>(i + 1U);
               ...
               probe.log[i].reg == RV3032::cmd::REG_CONTROL1) {
      cleanupControl1WriteOrdinal = static_cast<uint32_t>(i + 1U);
      break;
    }
  }
  TEST_ASSERT_NOT_EQUAL(0, cleanupPmuWriteOrdinal);
  TEST_ASSERT_NOT_EQUAL(0, cleanupControl1WriteOrdinal);
  ...
  FakeRv3032 reconciled;
  RV3032::RV3032 reconciledRtc;
  TEST_ASSERT_TRUE(reconciledRtc.begin(reconciled.config(true)).ok());
  TEST_ASSERT_TRUE(reconciledRtc.setClkoutConfig(config).inProgress());
  TEST_ASSERT_TRUE(pollJobToCompletion(reconciledRtc, reconciled).ok());
  reconciled.failOrdinal = cleanupControl1WriteOrdinal;
  reconciled.failWriteAfterCommit = true;
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::Err::EEPROM_CLEANUP_FAILED),
      static_cast<uint8_t>(
          pollEepromToCompletion(reconciledRtc, reconciled, 4).code));
  TEST_ASSERT_FALSE(
      reconciledRtc.getSettings().persistentAccessStateUnproven);
  TEST_ASSERT_EQUAL_UINT8(2, reconciledRtc.eepromQueueDepth());
  reconciled.failOrdinal = 0;
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::Err::EEPROM_CLEANUP_FAILED),
      static_cast<uint8_t>(
          pollEepromToCompletion(reconciledRtc, reconciled, 4).code));
  TEST_ASSERT_EQUAL_UINT8(0, reconciledRtc.eepromQueueDepth());
  TEST_ASSERT_FALSE(
      reconciledRtc.getSettings().persistentAccessStateUnproven);
}

void test_polling_predicates_drain_queued_work_behind_ordinary_job() {
  FakeRv3032 fake;
  RV3032::RV3032 rtc;
  TEST_ASSERT_FALSE(rtc.isEepromPollable());
  TEST_ASSERT_TRUE(rtc.begin(fake.config(true)).ok());
  TEST_ASSERT_FALSE(rtc.isEepromPollable());
  TEST_ASSERT_TRUE(rtc.setOffsetPpm(0.238418579f).inProgress());
  TEST_ASSERT_TRUE(pollJobToCompletion(rtc, fake).ok());
  TEST_ASSERT_TRUE(rtc.isEepromPollable());
  TEST_ASSERT_TRUE(rtc.setTrickleChargeResistance(
      RV3032::TrickleChargeResistance::KOHM_2).inProgress());
  TEST_ASSERT_TRUE(rtc.isOrdinaryJobBusy());
  TEST_ASSERT_TRUE(rtc.isEepromBusy());
  TEST_ASSERT_FALSE(rtc.isEepromPollable());
  const uint32_t before = fake.callbackCount;
  uint8_t used = 99;
  TEST_ASSERT_EQUAL_INT(static_cast<int>(RV3032::Err::BUSY),
      static_cast<int>(rtc.pollEeprom(fake.nowMs, 1, used).code));
  TEST_ASSERT_EQUAL_UINT8(0, used);
  TEST_ASSERT_EQUAL_UINT32(before, fake.callbackCount);
  for (uint32_t polls = 0; polls < 1000U &&
       (rtc.isOrdinaryJobBusy() || rtc.isEepromPollable()); ++polls) {
    const uint32_t callbacksBefore = fake.callbackCount;
    const RV3032::Status st = rtc.isOrdinaryJobBusy()
        ? rtc.pollJob(fake.nowMs, 1, used) : rtc.pollEeprom(fake.nowMs, 1, used);
    TEST_ASSERT_TRUE(st.ok() || st.inProgress());
    TEST_ASSERT_LESS_OR_EQUAL_UINT32(1, fake.callbackCount - callbacksBefore);
    ++fake.nowMs;
  }
  TEST_ASSERT_FALSE(rtc.isJobBusy());
  TEST_ASSERT_FALSE(rtc.isEepromBusy());
  TEST_ASSERT_FALSE(rtc.isEepromPollable());
  TEST_ASSERT_EQUAL_UINT32(2, rtc.eepromWriteCount());
  TEST_ASSERT_FALSE(fake.protocolViolation);
  TEST_ASSERT_FALSE(fake.passwordCommandAttempted);
}

void test_eeprom_queue_preserves_completed_ordinary_job_evidence() {
  FakeRv3032 fake;
  RV3032::RV3032 rtc;
  TEST_ASSERT_TRUE(rtc.begin(fake.config(true)).ok());
  ...
  TEST_ASSERT_TRUE(rtc.startSetBackupSwitchModeJob(
      RV3032::BackupSwitchMode::Direct, fake.nowMs).inProgress());
  const RV3032::Status failedJob = pollJobToCompletion(rtc, fake, 1);
  TEST_ASSERT_EQUAL_UINT8(static_cast<uint8_t>(RV3032::Err::I2C_BUS),
                          static_cast<uint8_t>(failedJob.code));
  RV3032::ConfigurationJobReport before{};
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::Err::I2C_BUS),
      static_cast<uint8_t>(
          rtc.getSetBackupSwitchModeJobResult(before).code));
  ...
  TEST_ASSERT_EQUAL_UINT8(static_cast<uint8_t>(RV3032::Err::I2C_BUS),
                          static_cast<uint8_t>(rtc.getJobStatus().code));
  RV3032::ConfigurationJobReport after{};
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::Err::I2C_BUS),
      static_cast<uint8_t>(
          rtc.getSetBackupSwitchModeJobResult(after).code));
  TEST_ASSERT_EQUAL_UINT8(static_cast<uint8_t>(before.operationStatus.code),
                          static_cast<uint8_t>(after.operationStatus.code));
  TEST_ASSERT_EQUAL_INT32(before.operationStatus.detail,
...
void test_backup_charge_policy_requires_explicit_rechargeable_intent() {
  for (auto mode : {RV3032::BackupSwitchMode::Direct,
                    RV3032::BackupSwitchMode::Level}) {
    FakeRv3032 fake;
    const uint8_t original = static_cast<uint8_t>(
        (mode == RV3032::BackupSwitchMode::Direct
             ? RV3032::cmd::PMU_BSM_DIRECT : RV3032::cmd::PMU_BSM_LEVEL) |
        static_cast<uint8_t>(RV3032::TrickleChargeMode::V3_0));
    fake.activeConfig[0] = original;
    RV3032::RV3032 rtc;
    TEST_ASSERT_TRUE(rtc.begin(fake.config(false)).ok());
    TEST_ASSERT_TRUE(rtc.startSetBackupSwitchModeJob(mode, fake.nowMs).inProgress());
    TEST_ASSERT_TRUE(pollJobToCompletion(rtc, fake).ok());
    TEST_ASSERT_EQUAL_UINT32(2, fake.callbackCount);
    TEST_ASSERT_EQUAL_UINT32(0, countWritesTo(fake, RV3032::cmd::REG_ACTIVE_PMU));
    TEST_ASSERT_EQUAL_HEX8(original, fake.activeConfig[0]);
    RV3032::ConfigurationJobReport report{};
    TEST_ASSERT_TRUE(rtc.getSetBackupSwitchModeJobResult(report).ok());
    TEST_ASSERT_FALSE(report.mutationAttempted);
  }
  const uint8_t inertCharger = 0x01;
  {
    FakeRv3032 fake;
    ...
    TEST_ASSERT_EQUAL_UINT8(static_cast<uint8_t>(RV3032::Err::INVALID_PARAM),
                            static_cast<uint8_t>(terminal.code));
    TEST_ASSERT_EQUAL_INT32(inertCharger, terminal.detail);
    TEST_ASSERT_EQUAL_UINT32(0,
        countWritesTo(fake, RV3032::cmd::REG_ACTIVE_PMU));
    RV3032::ConfigurationJobReport report{};
    TEST_ASSERT_EQUAL_UINT8(static_cast<uint8_t>(RV3032::Err::INVALID_PARAM),
  ...
  }
  {
    ...
    TEST_ASSERT_TRUE(pollJobToCompletion(rtc, fake, 1).ok());
    TEST_ASSERT_EQUAL_HEX8(
        static_cast<uint8_t>(inertCharger | RV3032::cmd::PMU_BSM_DIRECT),
        fake.activeConfig[0]);
  }

  const uint8_t enabledWithoutTcm = RV3032::cmd::PMU_BSM_DIRECT;
  {
    FakeRv3032 fake;
    ...
    TEST_ASSERT_EQUAL_UINT8(static_cast<uint8_t>(RV3032::Err::INVALID_PARAM),
                            static_cast<uint8_t>(terminal.code));
    TEST_ASSERT_EQUAL_INT32(enabledWithoutTcm, terminal.detail);
    TEST_ASSERT_EQUAL_UINT32(
  ...
  }
  {
    ...
    TEST_ASSERT_TRUE(pollJobToCompletion(rtc, fake, 1).ok());
    TEST_ASSERT_EQUAL_HEX8(
        static_cast<uint8_t>(enabledWithoutTcm |
            static_cast<uint8_t>(RV3032::TrickleChargeMode::V3_0)),
        fake.activeConfig[0]);
  }
}

void test_abandoned_access_cleanup_stays_latched_across_lifecycles() {
  for (bool generic : {false, true}) {
    ...
    uint8_t used = 0;
    const RV3032::Status st = generic
        ? rtc.pollEeprom(fake.nowMs, 2, used) : rtc.pollJob(fake.nowMs, 2, used);
    TEST_ASSERT_TRUE(st.inProgress());
    TEST_ASSERT_BITS_HIGH(RV3032::cmd::CONTROL1_EERD_MASK,
                          fake.direct[RV3032::cmd::REG_CONTROL1]);
    const uint32_t callbacksBefore = fake.callbackCount;
    ...
    TEST_ASSERT_TRUE(rtc.startPersistentAccessStateRecoveryJob(
        RV3032::cmd::PMU_BSM_LEVEL, fake.nowMs).inProgress());
    TEST_ASSERT_TRUE(pollJobToCompletion(rtc, fake).ok());
    TEST_ASSERT_FALSE(rtc.getSettings().persistentAccessStateUnproven);
  }
}

void test_access_recovery_restores_eerd_after_ready_failure() {
  for (bool busyTimeout : {false, true}) {
    FakeRv3032 fake;
    fake.direct[RV3032::cmd::REG_CONTROL1] = RV3032::cmd::CONTROL1_EERD_MASK;
    fake.activeConfig[0] = 0;
    RV3032::Config config = fake.config(false);
    config.eepromTimeoutMs = 10;
    RV3032::RV3032 rtc;
    TEST_ASSERT_TRUE(rtc.begin(config).ok());
    if (busyTimeout) fake.setInitialBusy(500);
    else fake.failOrdinal = 1;
    TEST_ASSERT_TRUE(rtc.startPersistentAccessStateRecoveryJob(
        RV3032::cmd::PMU_BSM_LEVEL, fake.nowMs).inProgress());
    const RV3032::Err expected = busyTimeout
        ? RV3032::Err::TIMEOUT : RV3032::Err::I2C_BUS;
    TEST_ASSERT_EQUAL_INT(static_cast<int>(expected),
        static_cast<int>(pollJobToCompletion(rtc, fake, 1).code));
    RV3032::ConfigurationJobReport report{};
    TEST_ASSERT_EQUAL_INT(static_cast<int>(expected), static_cast<int>(
        rtc.getPersistentAccessStateRecoveryJobResult(report).code));
    TEST_ASSERT_EQUAL_INT(static_cast<int>(expected),
                          static_cast<int>(report.operationStatus.code));
    TEST_ASSERT_TRUE(report.cleanupStatus.ok());
    TEST_ASSERT_EQUAL_INT(static_cast<int>(
        RV3032::ConfigurationFinalState::REQUESTED_VERIFIED),
        static_cast<int>(report.finalState));
    TEST_ASSERT_BITS_LOW(RV3032::cmd::CONTROL1_EERD_MASK,
                         fake.direct[RV3032::cmd::REG_CONTROL1]);
    TEST_ASSERT_EQUAL_HEX8(RV3032::cmd::PMU_BSM_LEVEL, fake.activeConfig[0]);
    TEST_ASSERT_FALSE(rtc.getSettings().persistentAccessStateUnproven);
    TEST_ASSERT_EQUAL_UINT16(0, fake.writeOneAttempts);
    TEST_ASSERT_EQUAL_UINT16(0, fake.readOneAttempts);
    TEST_ASSERT_FALSE(fake.protocolViolation);
...
void test_persistent_access_state_recovery_is_explicit_and_proven() {
  FakeRv3032 fake;
  const uint8_t preservedControl1 = 0x3B;
  fake.direct[RV3032::cmd::REG_CONTROL1] = preservedControl1;
  RV3032::RV3032 rtc;
  TEST_ASSERT_TRUE(rtc.begin(fake.config(true)).ok());
  TEST_ASSERT_TRUE(rtc.setOffsetPpm(0.238418579f).inProgress());
  TEST_ASSERT_TRUE(pollJobToCompletion(rtc, fake, 1).ok());
  ...
  TEST_ASSERT_EQUAL_UINT8(
      ...
      static_cast<uint8_t>(rtc.pollEeprom(fake.nowMs, 1, used).code));
  RV3032::SettingsSnapshot snapshot = rtc.getSettings();
  TEST_ASSERT_TRUE(snapshot.persistentAccessStateUnproven);

  const uint32_t callbacksBeforeRebind = fake.callbackCount;
  rtc.end();
  TEST_ASSERT_TRUE(rtc.begin(fake.config(false)).ok());
  TEST_ASSERT_EQUAL_UINT32(callbacksBeforeRebind, fake.callbackCount);
  TEST_ASSERT_TRUE(rtc.getSettings().persistentAccessStateUnproven);
  TEST_ASSERT_BITS_HIGH(RV3032::cmd::CONTROL1_EERD_MASK,
                        fake.direct[RV3032::cmd::REG_CONTROL1]);

  RV3032::PrimaryCellConfigurationReport blockedEnsure{};
  const uint32_t callbacksBeforeRejectedEnsure = fake.callbackCount;
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::Err::EEPROM_CLEANUP_FAILED),
      static_cast<uint8_t>(
          rtc.ensurePrimaryCellConfiguration(blockedEnsure).code));
  TEST_ASSERT_EQUAL_UINT32(callbacksBeforeRejectedEnsure, fake.callbackCount);
  TEST_ASSERT_FALSE(rtc.getSettings().primaryCellEnsureAttempted);

  const uint32_t callbacksBeforeRejectedRead = fake.callbackCount;
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::Err::EEPROM_CLEANUP_FAILED),
      static_cast<uint8_t>(rtc.startReadConfigurationEepromJob(
          RV3032::ConfigurationEepromRegister::PMU,
          fake.nowMs).code));
  TEST_ASSERT_EQUAL_UINT32(callbacksBeforeRejectedRead, fake.callbackCount);

  const uint8_t desiredC0 = RV3032::cmd::PMU_BSM_LEVEL;
  const uint32_t commandsBeforeRecovery =
      countEepromCommands(fake, RV3032::cmd::EEPROM_CMD_WRITE_ONE) +
      countEepromCommands(fake, RV3032::cmd::EEPROM_CMD_READ_ONE);
  fake.failOrdinal = fake.callbackCount + 2U;  // Control 1 needed for restoration.
  TEST_ASSERT_TRUE(rtc.startPersistentAccessStateRecoveryJob(
      desiredC0, fake.nowMs).inProgress());
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::Err::I2C_BUS),
      static_cast<uint8_t>(pollJobToCompletion(rtc, fake, 1).code));
  TEST_ASSERT_TRUE(rtc.getSettings().persistentAccessStateUnproven);
  RV3032::ConfigurationJobReport failedReport{};
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::Err::I2C_BUS),
      static_cast<uint8_t>(
          rtc.getPersistentAccessStateRecoveryJobResult(failedReport).code));
  TEST_ASSERT_FALSE(failedReport.mutationAttempted);
  ...
  RV3032::ConfigurationJobReport report{};
  TEST_ASSERT_TRUE(
      rtc.getPersistentAccessStateRecoveryJobResult(report).ok());
  TEST_ASSERT_TRUE(report.mutationAttempted);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::ConfigurationFinalState::REQUESTED_VERIFIED),
      static_cast<uint8_t>(report.finalState));
  TEST_ASSERT_EQUAL_HEX8(desiredC0, fake.activeConfig[0]);
  TEST_ASSERT_EQUAL_HEX8(
      0, static_cast<uint8_t>(fake.direct[RV3032::cmd::REG_CONTROL1] &
                              RV3032::cmd::CONTROL1_EERD_MASK));
  TEST_ASSERT_EQUAL_HEX8(
      preservedControl1,
      static_cast<uint8_t>(fake.direct[RV3032::cmd::REG_CONTROL1] &
                           RV3032::cmd::CONTROL1_IMPLEMENTED_MASK));
  TEST_ASSERT_EQUAL_UINT32(
      ...
      countEepromCommands(fake, RV3032::cmd::EEPROM_CMD_WRITE_ONE) +
          countEepromCommands(fake, RV3032::cmd::EEPROM_CMD_READ_ONE));
  snapshot = rtc.getSettings();
  TEST_ASSERT_FALSE(snapshot.persistentAccessStateUnproven);

  FakeRv3032 admissionFake;
  RV3032::RV3032 admissionRtc;
  TEST_ASSERT_TRUE(admissionRtc.begin(admissionFake.config()).ok());
  const uint32_t admissionCallbacks = admissionFake.callbackCount;
  const uint8_t chargingC0 = static_cast<uint8_t>(
      RV3032::cmd::PMU_BSM_DIRECT |
      static_cast<uint8_t>(RV3032::TrickleChargeMode::V1_75));
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::Err::INVALID_PARAM),
      static_cast<uint8_t>(admissionRtc.startPersistentAccessStateRecoveryJob(
          chargingC0, admissionFake.nowMs).code));
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::Err::INVALID_PARAM),
      static_cast<uint8_t>(admissionRtc.startPersistentAccessStateRecoveryJob(
          0x80, admissionFake.nowMs).code));
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::Err::INVALID_PARAM),
      static_cast<uint8_t>(admissionRtc.startPersistentAccessStateRecoveryJob(
          0, admissionFake.nowMs, 125).code));
  TEST_ASSERT_EQUAL_UINT32(admissionCallbacks,
                           admissionFake.callbackCount);
  TEST_ASSERT_TRUE(admissionRtc.startPersistentAccessStateRecoveryJob(
      chargingC0, admissionFake.nowMs,
...
void test_persistent_deadlines_stop_callbacks_and_report_unverified_cleanup() {
  ...
  FakeRv3032 preexistingEerd;
  preexistingEerd.direct[RV3032::cmd::REG_CONTROL1] =
      RV3032::cmd::CONTROL1_EERD_MASK;
  RV3032::RV3032 preexistingEerdRtc;
  TEST_ASSERT_TRUE(preexistingEerdRtc.begin(preexistingEerd.config()).ok());
  TEST_ASSERT_TRUE(preexistingEerdRtc.startReadConfigurationEepromJob(
      RV3032::ConfigurationEepromRegister::PMU,
      preexistingEerd.nowMs, 400).inProgress());
...
void test_recovery_settle_timeout_preserves_proven_access_state() {
  ...
  const uint32_t recoveryStartMs = 410;
...
void test_persistent_settle_timeout_preserves_proven_cleanup() {
  ...
  TEST_ASSERT_TRUE(rtc.startReadConfigurationEepromJob(
      RV3032::ConfigurationEepromRegister::OFFSET,
      fake.nowMs, 400).inProgress());
  while (fake.callbackCount + 1U < finalControl1Read) {
    uint8_t used = 0;
    TEST_ASSERT_TRUE(rtc.pollJob(fake.nowMs, 1, used).inProgress());
    if (used == 0) ++fake.nowMs;
  }
  fake.nowMs = 395;
  while (fake.callbackCount < finalControl1Read) {
    uint8_t used = 0;
    TEST_ASSERT_TRUE(rtc.pollJob(fake.nowMs, 1, used).inProgress());
  }
  const uint32_t callbacksAfterProof = fake.callbackCount;
  ...
  for (DeadlinePoint point : points) {
    for (uint32_t lateMs = 0; lateMs <= 1; ++lateMs) {
      ...
      if (point == DeadlinePoint::CLEANUP_READY) {
        ...
        TEST_ASSERT_FALSE(rtc.isJobBusy());
        RV3032::PersistentReadResult result{};
        TEST_ASSERT_EQUAL_INT(static_cast<int>(RV3032::Err::EEPROM_CLEANUP_FAILED),
            static_cast<int>(rtc.getPersistentReadJobResult(result).code));
        TEST_ASSERT_TRUE(result.cleanupVerified);
        TEST_ASSERT_FALSE(rtc.getSettings().persistentAccessStateUnproven);
      } else {
        TEST_ASSERT_TRUE(boundary.inProgress());
...
void test_dispatched_write_one_crossing_cutoff_proves_durability_without_replay() {
  FakeRv3032 baseline;
  RV3032::RV3032 baselineRtc;
  ...
  for (uint32_t lateMs = 0; lateMs <= 1; ++lateMs) {
    ...
    // timeout=1000, complete post-write reserve=510, cutoff=490.
    ...
    TEST_ASSERT_TRUE(rtc.pollJob(fake.nowMs, 1, used).inProgress());
    TEST_ASSERT_EQUAL_UINT8(1, used);
    TEST_ASSERT_EQUAL_UINT32(490U + lateMs, fake.nowMs);
    ...
    TEST_ASSERT_EQUAL_INT(static_cast<int>(RV3032::Err::I2C_TIMEOUT),
...
void test_smallest_persistent_budgets_execute() {
  for (uint8_t instructionBudget : {uint8_t{1}, uint8_t{255}}) {
    for (bool clockHook : {false, true}) {
      for (uint32_t callbackTimeout : {1U, 5U, 50U, 100U}) {
        for (uint32_t eepromTimeout : {10U, 100U, 250U}) {
          for (uint32_t duration : {0U, callbackTimeout}) {
            // Clocked admission permits fast callbacks; it does not promise
            // completion when every callback consumes its maximum timeout.
            if (clockHook && duration != 0U) continue;
            for (uint32_t start : {0U, UINT32_MAX - 10U}) {
              for (bool write : {false, true}) {
                FakeRv3032 fake;
                fake.nowMs = start;
                fake.callbackDurationMs = duration;
                RV3032::Config config = fake.config(write);
                config.i2cTimeoutMs = callbackTimeout;
                config.eepromTimeoutMs = eepromTimeout;
                if (!clockHook) config.nowMs = nullptr;
                RV3032::RV3032 rtc;
                TEST_ASSERT_TRUE(rtc.begin(config).ok());
                const uint32_t cleanup = 250U + 6U * callbackTimeout + 10U;
                const uint32_t reserve = write
                    ? cleanup + 10U + eepromTimeout + 12U * callbackTimeout + 50U
                    : cleanup;
                const uint32_t minimum = reserve +
                    (clockHook ? 1U : (write ? 27U : 21U)) * callbackTimeout + 3U;
                const uint8_t desired = 0x5A;
                auto admit = [&](uint32_t budget) {
                  return write
                      ? rtc.startWriteUserEepromJob(0, &desired, 1, start, budget)
                      : rtc.startReadUserEepromJob(0, 1, start, budget);
                };
                TEST_ASSERT_EQUAL_INT(static_cast<int>(RV3032::Err::INVALID_PARAM),
                    static_cast<int>(admit(minimum - 1U).code));
                RV3032::Status st = admit(minimum);
                TEST_ASSERT_TRUE(st.inProgress());
                TEST_ASSERT_EQUAL_UINT32(0, fake.callbackCount);
                for (uint32_t polls = 0; st.inProgress() && polls < 20000U; ++polls) {
                  const auto state = RV3032::NativeTestAccess::job(rtc).persistent.state;
                  const uint32_t callbacks = fake.callbackCount;
                  uint8_t used = 0;
                  st = rtc.pollJob(fake.nowMs, instructionBudget, used);
                  TEST_ASSERT_LESS_OR_EQUAL_UINT8(instructionBudget, used);
                  TEST_ASSERT_EQUAL_UINT32(used, fake.callbackCount - callbacks);
                  // Immediate owner scheduling: only advance idle vendor waits.
                  if (st.inProgress() && used == 0 &&
                      RV3032::NativeTestAccess::job(rtc).persistent.state == state) {
                    ++fake.nowMs;
                  }
                }
                TEST_ASSERT_EQUAL_INT_MESSAGE(static_cast<int>(RV3032::Err::OK),
                    static_cast<int>(st.code), st.msg);
                TEST_ASSERT_LESS_OR_EQUAL_UINT32(minimum, fake.nowMs - start);
                if (write) {
                  RV3032::UserEepromWriteReport report{};
                  TEST_ASSERT_TRUE(rtc.getUserEepromWriteJobResult(report).ok());
                  TEST_ASSERT_EQUAL_UINT8(1, report.durablyVerifiedBytes);
                  TEST_ASSERT_TRUE(report.cleanupVerified);
                  TEST_ASSERT_EQUAL_UINT16(1, fake.writeOneAttempts);
                  TEST_ASSERT_EQUAL_HEX8(desired,
                      fake.persistent[RV3032::cmd::USER_EEPROM_START -
                                      RV3032::cmd::CONFIG_EEPROM_START]);
                } else {
                  RV3032::PersistentReadResult result{};
...
void test_persistent_default_budgets_execute_across_transport_limits() {
  for (bool clockHook : {false, true}) {
    for (uint32_t callbackTimeout : {1U, 100U}) {
      for (uint32_t eepromTimeout : {10U, 100U, 250U}) {
        for (uint32_t duration : {0U, callbackTimeout}) {
          for (uint8_t instructionBudget : {uint8_t{1}, uint8_t{255}}) {
            for (uint32_t start : {0U, UINT32_MAX - 10U}) {
              for (uint32_t initiallyBusyMs : {0U, 200U}) {
                // Configuration read, user read, user write, queued offset.
                for (uint8_t surface = 0; surface < 4; ++surface) {
                  FakeRv3032 fake;
                  fake.nowMs = start;
                  fake.callbackDurationMs = duration;
                  RV3032::Config config = fake.config(surface >= 2);
                  config.i2cTimeoutMs = callbackTimeout;
                  config.eepromTimeoutMs = eepromTimeout;
                  if (!clockHook) config.nowMs = nullptr;
                  RV3032::RV3032 rtc;
                  ...
                  if (surface == 3) {
                    ...
                    TEST_ASSERT_TRUE(pollJobToCompletion(
                        rtc, fake, instructionBudget).ok());
                    TEST_ASSERT_EQUAL_UINT8(1, rtc.eepromQueueDepth());
                  }
                  const uint32_t operationStart = fake.nowMs;
                  fake.setInitialBusy(initiallyBusyMs);
                  if (surface == 0) {
                    st = rtc.startReadConfigurationEepromJob(
                        RV3032::ConfigurationEepromRegister::PMU, fake.nowMs);
                  } else if (surface == 1) {
                    st = rtc.startReadUserEepromJob(0, 1, fake.nowMs);
                  } else if (surface == 2) {
                    st = rtc.startWriteUserEepromJob(0, &desired, 1, fake.nowMs);
                  } else {
                    st = rtc.getEepromStatus();
                  }
                  TEST_ASSERT_TRUE(st.inProgress());
                  st = surface == 3
                      ? pollEepromToCompletion(rtc, fake, instructionBudget, 10000)
                      : pollJobToCompletion(rtc, fake, instructionBudget, 10000);
                  TEST_ASSERT_EQUAL_INT_MESSAGE(static_cast<int>(RV3032::Err::OK),
                      static_cast<int>(st.code), st.msg);
                  const uint32_t completeBound = surface == 2 ? 6000U
                      : surface == 3 ? 5323U : 4000U;
                  TEST_ASSERT_LESS_OR_EQUAL_UINT32(
                      completeBound, fake.nowMs - operationStart);
                  if (surface < 2) {
                    RV3032::PersistentReadResult result{};
                    TEST_ASSERT_TRUE(rtc.getPersistentReadJobResult(result).ok());
                    TEST_ASSERT_EQUAL_UINT8(1, result.length);
                    ...
                    TEST_ASSERT_EQUAL_UINT32(0, rtc.eepromWriteFailures());
                    TEST_ASSERT_EQUAL_UINT16(1, fake.writeOneAttempts);
                    TEST_ASSERT_EQUAL_HEX8(1, fake.persistent[1]);
                    TEST_ASSERT_EQUAL_HEX8(1, fake.activeConfig[1]);
                  }
                  TEST_ASSERT_EQUAL_HEX8(0, fake.direct[RV3032::cmd::REG_CONTROL1] &
                      RV3032::cmd::CONTROL1_EERD_MASK);
                  TEST_ASSERT_EQUAL_HEX8(RV3032::cmd::PMU_BSM_LEVEL, fake.activeConfig[0]);
                  TEST_ASSERT_FALSE(rtc.getSettings().persistentAccessStateUnproven);
                  TEST_ASSERT_FALSE(fake.protocolViolation);
...
void test_cleanup_busy_phase_expiry_still_restores_access_state() {
  for (bool queued : {false, true}) {
    for (bool checkCap : {false, true}) {
      for (uint32_t start : {0U, UINT32_MAX - 10U}) {
        FakeRv3032 fake;
        fake.nowMs = start;
        RV3032::RV3032 rtc;
        ...
        if (queued) {
          ...
          const uint8_t desired = 0x5A;
          TEST_ASSERT_TRUE(rtc.startWriteUserEepromJob(
              0, &desired, 1, fake.nowMs).inProgress());
        }
        // Fail address staging only after EERD and safe C0 are proven.
        fake.failOrdinal = fake.callbackCount + 8U;
        auto& op = queued ? RV3032::NativeTestAccess::eeprom(rtc).persistent
                          : RV3032::NativeTestAccess::job(rtc).persistent;
        uint8_t used = 0;
        for (uint8_t polls = 0; polls < 20 &&
             op.state != RV3032::NativeTestAccess::EepromState::CLEANUP_WAIT_READY;
             ++polls) {
          const RV3032::Status st = queued
              ? rtc.pollEeprom(fake.nowMs, 1, used)
              : rtc.pollJob(fake.nowMs, 1, used);
          TEST_ASSERT_TRUE(st.inProgress());
        }
        TEST_ASSERT_TRUE(op.state ==
            RV3032::NativeTestAccess::EepromState::CLEANUP_WAIT_READY);
        fake.setInitialBusy(10000);
        if (checkCap) op.readyChecks = 256;
        else fake.nowMs = op.phaseDeadlineMs;
        const uint32_t beforeExpiry = fake.callbackCount;
        const RV3032::Status boundary = queued
            ? rtc.pollEeprom(fake.nowMs, 1, used)
            : rtc.pollJob(fake.nowMs, 1, used);
        TEST_ASSERT_TRUE(boundary.inProgress());
        TEST_ASSERT_EQUAL_UINT8(0, used);
        TEST_ASSERT_EQUAL_UINT32(beforeExpiry, fake.callbackCount);
        const RV3032::Status terminal = queued
            ? pollEepromToCompletion(rtc, fake, 1)
            : pollJobToCompletion(rtc, fake, 1);
        TEST_ASSERT_EQUAL_INT(static_cast<int>(RV3032::Err::EEPROM_CLEANUP_FAILED),
            static_cast<int>(terminal.code));
        if (queued) {
          const auto settings = rtc.getSettings();
          TEST_ASSERT_EQUAL_INT(static_cast<int>(RV3032::Err::I2C_BUS),
              static_cast<int>(settings.eepromOperationStatus.code));
          TEST_ASSERT_EQUAL_INT(static_cast<int>(RV3032::Err::TIMEOUT),
              static_cast<int>(settings.eepromCleanupStatus.code));
          TEST_ASSERT_EQUAL_HEX8(1, fake.activeConfig[1]);
        } else {
          RV3032::UserEepromWriteReport report{};
          TEST_ASSERT_EQUAL_INT(static_cast<int>(RV3032::Err::EEPROM_CLEANUP_FAILED),
              static_cast<int>(rtc.getUserEepromWriteJobResult(report).code));
          TEST_ASSERT_TRUE(report.cleanupVerified);
          TEST_ASSERT_EQUAL_UINT8(0, report.durablyVerifiedBytes);
          TEST_ASSERT_EQUAL_INT(static_cast<int>(RV3032::Err::I2C_BUS),
              static_cast<int>(report.operationStatus.code));
          TEST_ASSERT_EQUAL_INT(static_cast<int>(RV3032::Err::TIMEOUT),
              static_cast<int>(report.cleanupStatus.code));
        }
        TEST_ASSERT_FALSE(rtc.getSettings().persistentAccessStateUnproven);
        TEST_ASSERT_EQUAL_HEX8(0, fake.direct[RV3032::cmd::REG_CONTROL1] &
            RV3032::cmd::CONTROL1_EERD_MASK);
...
void test_fixture_eeprom_command_and_transfer_evidence() {
  FakeRv3032 fake;
  ...
  FakeRv3032 password;
  password.activeConfig[0] = RV3032::cmd::PMU_BSM_DISABLED;
  password.direct[RV3032::cmd::REG_CONTROL1] =
      RV3032::cmd::CONTROL1_EERD_MASK;
  const uint8_t passwordIndex = static_cast<uint8_t>(
      RV3032::cmd::REG_EEPROM_PASSWORD0 -
      RV3032::cmd::CONFIG_EEPROM_START);
  const uint8_t persistentBefore = password.persistent[passwordIndex];
  const uint8_t addressWrite[2] = {
      RV3032::cmd::REG_EE_ADDRESS,
      RV3032::cmd::REG_EEPROM_PASSWORD0};
  const uint8_t dataWrite[2] = {RV3032::cmd::REG_EE_DATA, 0xA5};
  const uint8_t passwordCommand[2] = {
      RV3032::cmd::REG_EE_COMMAND,
      RV3032::cmd::EEPROM_CMD_WRITE_ONE};
  TEST_ASSERT_TRUE(FakeRv3032::writeCallback(
      RV3032::cmd::I2C_ADDR_7BIT, addressWrite, sizeof(addressWrite), 5,
      &password).ok());
  TEST_ASSERT_TRUE(FakeRv3032::writeCallback(
      RV3032::cmd::I2C_ADDR_7BIT, dataWrite, sizeof(dataWrite), 5,
      &password).ok());
  TEST_ASSERT_TRUE(FakeRv3032::writeCallback(
      RV3032::cmd::I2C_ADDR_7BIT, passwordCommand, sizeof(passwordCommand), 5,
      &password).ok());
  TEST_ASSERT_TRUE(password.passwordCommandAttempted);
  TEST_ASSERT_TRUE(password.protocolViolation);
  TEST_ASSERT_BITS_HIGH(RV3032::cmd::EEPROM_EEF_MASK,
                        password.direct[RV3032::cmd::REG_TEMP_LSB]);
  TEST_ASSERT_EQUAL_HEX8(persistentBefore,
                         password.persistent[passwordIndex]);
  TEST_ASSERT_EQUAL_UINT16(0, password.writeOneAttempts);
}
...
void test_primary_phase4_fault_attribution_and_dispatch_evidence() {
  ...
  const uint32_t writeDataStagingOrdinal =
      findWriteDataStagingOrdinal(writeBaseline);
  TEST_ASSERT_NOT_EQUAL(0, writeDataStagingOrdinal);

  {
    FakeRv3032 fake;
    fake.persistent[0] = 0;
    fake.resetFromPersistent();
    fake.failOrdinal = writeDataStagingOrdinal;
    fake.failError = RV3032::Err::I2C_BUS;
    RV3032::RV3032 rtc;
    TEST_ASSERT_TRUE(rtc.begin(fake.config()).ok());
    RV3032::PrimaryCellConfigurationReport report{};
    ...
    TEST_ASSERT_EQUAL_UINT8(
        static_cast<uint8_t>(
            RV3032::PrimaryCellFailureStage::WRITE_PERSISTENT),
        static_cast<uint8_t>(report.failureStage));
    TEST_ASSERT_FALSE(report.writeCommandAttempted);
    TEST_ASSERT_EQUAL_UINT32(0, countEepromCommands(
        fake, RV3032::cmd::EEPROM_CMD_WRITE_ONE));
...
uint8_t phase2ConfigurationWorstCap(Phase2ConfigurationPath path) {
  ...
  switch (path) {
    case Phase2ConfigurationPath::TIMER:
      return Access::noWaitCallbackCap(Access::JobKind::SET_TIMER);
    case Phase2ConfigurationPath::PERIODIC:
      return Access::noWaitCallbackCap(Access::JobKind::SET_PERIODIC_UPDATE);
    case Phase2ConfigurationPath::CLKOUT:
      return Access::noWaitCallbackCap(Access::JobKind::SET_CLKOUT_CONFIG);
    case Phase2ConfigurationPath::TEMPERATURE:
      return Access::noWaitCallbackCap(Access::JobKind::SET_TEMPERATURE_EVENT_CONFIG);
    default: return 0;
  }
}
...
void test_staged_callback_time_bounds_at_maximum_transfer_timeout() {
  for (auto path : {Phase2ConfigurationPath::TIMER,
                    Phase2ConfigurationPath::PERIODIC,
                    Phase2ConfigurationPath::CLKOUT,
                    Phase2ConfigurationPath::TEMPERATURE}) {
    for (bool failFinalRead : {false, true}) {
      for (bool clockHook : {false, true}) {
        FakeRv3032 fake;
        RV3032::Config config = fake.config(false);
        config.i2cTimeoutMs = 100;
        fake.callbackDurationMs = config.i2cTimeoutMs;
        if (!clockHook) config.nowMs = nullptr;
        if (failFinalRead) fake.failOrdinal = phase2ConfigurationSuccessCap(path);
        RV3032::RV3032 rtc;
        TEST_ASSERT_TRUE(rtc.begin(config).ok());
        TEST_ASSERT_TRUE(startPhase2ConfigurationPath(path, rtc).inProgress());
        RV3032::Status st;
        for (uint8_t polls = 0; polls < 16U && rtc.isOrdinaryJobBusy(); ++polls) {
          uint8_t used = 0;
          st = rtc.pollJob(fake.nowMs, 1, used);
          TEST_ASSERT_LESS_OR_EQUAL_UINT8(1, used);
        }
        TEST_ASSERT_FALSE(rtc.isOrdinaryJobBusy());
        const uint8_t expectedCallbacks = failFinalRead
            ? phase2ConfigurationWorstCap(path) : phase2ConfigurationSuccessCap(path);
        TEST_ASSERT_EQUAL_UINT32(expectedCallbacks, fake.callbackCount);
        TEST_ASSERT_EQUAL_UINT32(expectedCallbacks * config.i2cTimeoutMs, fake.nowMs);
        TEST_ASSERT_EQUAL_INT(static_cast<int>(failFinalRead
            ? RV3032::Err::I2C_BUS : RV3032::Err::OK), static_cast<int>(st.code));
        RV3032::ConfigurationJobReport report{};
        TEST_ASSERT_EQUAL_INT(static_cast<int>(st.code), static_cast<int>(
...
void test_phase3_cli_invalid_mutating_commands_are_zero_io() {
  ...
  Serial.resetOutput();
  process_command(String("reg 0x40 1 confirm"));
  TEST_ASSERT_NOT_EQUAL(
      std::string::npos,
      Serial.output().find("User RAM writes do not use confirm"));
  TEST_ASSERT_EQUAL_UINT32(0, fake.callbackCount);
...
void test_phase3_cli_persistent_helper_deadline_does_not_orphan() {
  ...
  const uint32_t callbacksBeforeInvalidBounds = Wire.beginTransmissionCalls;
  uint8_t invalidValue = 0;
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::Err::INVALID_PARAM),
      static_cast<uint8_t>(read_user_eeprom_chunk(
          0, RV3032::USER_EEPROM_JOB_MAX_BYTES + 1U,
          &invalidValue).code));
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<uint8_t>(RV3032::Err::INVALID_PARAM),
      static_cast<uint8_t>(read_user_eeprom_chunk(0, 1, nullptr).code));
  TEST_ASSERT_EQUAL_UINT32(callbacksBeforeInvalidBounds,
                           Wire.beginTransmissionCalls);
...
void test_phase3_wire_validation_and_closed_status_domain() {
  ...
  const InvalidWriteCase invalidWrites[] = {
      ...
      {tx, transport::MAX_TRANSFER_BYTES + 1U, 5, &wire,
  ...
  const InvalidReadCase invalidReads[] = {
      ...
      {tx, transport::MAX_TRANSFER_BYTES + 1U, rx, 1, 5, &wire,
       transport::I2C_DETAIL_INVALID_ARGUMENT},
      {tx, 1, rx, transport::MAX_TRANSFER_BYTES + 1U, 5, &wire,
       transport::I2C_DETAIL_INVALID_ARGUMENT},
      {tx, 1, rx, 1, 5, nullptr,
       transport::I2C_DETAIL_INVALID_ARGUMENT},
  ...
  for (uint8_t result = 1; result <= 7; ++result) {
    ...
    TEST_ASSERT_EQUAL_UINT32(1, wire.flushCalls);
    TEST_ASSERT_EQUAL_UINT32(1, wire.physicalAttemptCalls);
    TEST_ASSERT_EQUAL_UINT32(0, wire.lastPhysicalTxLength);
  ...
  uint8_t unsupportedSpan[57] = {};
  TEST_ASSERT_EQUAL_INT(static_cast<int>(RV3032::Err::INVALID_PARAM),
      static_cast<int>(rtc.readRegisters(0, unsupportedSpan,
                                         sizeof(unsupportedSpan)).code));
  TEST_ASSERT_EQUAL_UINT32(1, wire.physicalAttemptCalls);
}
...
void test_phase3_wire_complete_deadline_timeout_restoration_and_order() {
  ...
  TEST_ASSERT_EQUAL_UINT32(1, wire.physicalAttemptCalls);
  TEST_ASSERT_EQUAL_UINT32(sizeof(tx), wire.lastPhysicalTxLength);
  TEST_ASSERT_EQUAL_UINT8_ARRAY(tx, wire.lastPhysicalTxData, sizeof(tx));
  TEST_ASSERT_EQUAL_UINT32(0, wire.flushCalls);
  TEST_ASSERT_FALSE(wire.transactionActive);
  ...
  TEST_ASSERT_EQUAL_UINT32(1, wire.physicalAttemptCalls);
  TEST_ASSERT_EQUAL_UINT32(1, wire.lastPhysicalTxLength);
  TEST_ASSERT_EQUAL_HEX8(tx[0], wire.lastPhysicalTxData[0]);
  TEST_ASSERT_EQUAL_UINT32(0, wire.flushCalls);
...
void test_phase3_wire_short_stage_release_and_initialization() {
  ...
  scannerWire.queueEndResult(0);
  scannerWire.queueEndResult(5);
  scannerWire.queueEndResult(2);
  Serial.resetOutput();
  ...
  TEST_ASSERT_EQUAL_UINT32(112, scannerWire.physicalAttemptCalls);
  TEST_ASSERT_EQUAL_UINT16(37, scannerWire.getTimeOut());
  TEST_ASSERT_NOT_EQUAL(std::string::npos,
                        Serial.output().find("08 TO --"));
  TEST_ASSERT_NOT_EQUAL(
      std::string::npos,
      Serial.output().find("Scan complete. Found 110 device(s)."));
}

void test_phase3_strict_cli_numeric_tokens_preserve_outputs() {
  TEST_ASSERT_EQUAL_STRING(
      "unknown", clkout_frequency_name(
                     static_cast<RV3032::ClkoutFrequency>(0xFF)));
...
void test_cli_pending_warning_and_serial_discard_preserve_owner() {
  FakeRv3032 fake;
  beginCliHarness(fake, false);
  arduinoStubMillis = UINT32_MAX - 10000U;
  const uint32_t startMs = arduinoStubMillis;
  TEST_ASSERT_TRUE(operationAccepted("timer configuration",
      g_rtc.setTimer(50, RV3032::TimerFrequency::Hz1, true)));
  TEST_ASSERT_EQUAL_UINT32(startMs, g_pendingOperation.startedMs);
  Serial.inject("verbose 1\nclear_porf");
  arduinoStubMillis = startMs + PENDING_WARNING_MS - 1U;
  loop();
  TEST_ASSERT_EQUAL_INT(0, Serial.available());
  TEST_ASSERT_FALSE(g_verbose);
  TEST_ASSERT_TRUE(g_rtc.isOrdinaryJobBusy());
  TEST_ASSERT_EQUAL_UINT32(1, fake.callbackCount);
  TEST_ASSERT_TRUE(Serial.output().find("exceeded 15 s") == std::string::npos);
  loop();
  TEST_ASSERT_TRUE(g_rtc.isOrdinaryJobBusy());
  TEST_ASSERT_TRUE(g_pendingOperation.overdueReported);
  const std::string warning = Serial.output();
  TEST_ASSERT_TRUE(warning.find("Pending ordinary job operation 'timer configuration'")
                   != std::string::npos);
  TEST_ASSERT_TRUE(warning.find("IN_PROGRESS") != std::string::npos);
  loop();
  TEST_ASSERT_EQUAL_STRING(warning.c_str(), Serial.output().c_str());
  driveCliPending(fake);
  const uint32_t before = fake.callbackCount;
  Serial.inject("\nverbose 1\n");
  loop();
  TEST_ASSERT_TRUE(g_verbose);
  TEST_ASSERT_EQUAL_UINT32(before, fake.callbackCount);
  TEST_ASSERT_FALSE(g_pendingOperation.overdueReported);
}
...
void test_cli_terminal_callback_input_is_discarded_on_both_surfaces() {
  struct InputInjectingFake : FakeRv3032 {
    const char* callbackInput = nullptr;
    static RV3032::Status readWithInput(
        uint8_t address, const uint8_t* tx, size_t txLen,
        uint8_t* rx, size_t rxLen, uint32_t timeoutMs, void* user) {
      auto& fake = *static_cast<InputInjectingFake*>(user);
      const RV3032::Status status = FakeRv3032::readCallback(
          address, tx, txLen, rx, rxLen, timeoutMs, user);
      if (fake.callbackInput != nullptr) {
        Serial.inject(fake.callbackInput);
        fake.callbackInput = nullptr;
      }
      return status;
    }
  };
  for (bool genericEeprom : {false, true}) {
    for (size_t oldBacklog : {size_t{0},
                             2U * cli_shell::MAX_INPUT_BYTES_PER_POLL}) {
      InputInjectingFake fake;
      beginCliHarness(fake, genericEeprom);
      RV3032::Config config = fake.config(genericEeprom);
      config.i2cWriteRead = InputInjectingFake::readWithInput;
      g_rtc.end();
      TEST_ASSERT_TRUE(g_rtc.begin(config).ok());
      if (genericEeprom) {
        TEST_ASSERT_TRUE(operationAccepted(
            "frequency offset", g_rtc.setOffsetPpm(1.0f)));
        driveCliPending(fake, PendingSurface::EEPROM);
        // The first generic read failure is terminal with no cleanup needed.
        fake.failOrdinal = fake.callbackCount + 1U;
      } else {
        TEST_ASSERT_TRUE(operationAccepted("timer configuration",
            g_rtc.setTimer(50, RV3032::TimerFrequency::Hz1, true)));
        for (uint8_t i = 0; i < 5; ++i) pollPendingOperation(fake.nowMs);
      }
      TEST_ASSERT_EQUAL_INT(static_cast<int>(genericEeprom
          ? PendingSurface::EEPROM : PendingSurface::ORDINARY_JOB),
          static_cast<int>(g_pendingOperation.surface));
      fake.callbackInput = "verbose 1\nclear_porf";
      Serial.inject(std::string(oldBacklog, '\n'));
      const size_t injectedLength = std::strlen(fake.callbackInput);
      const uint32_t before = fake.callbackCount;
      loop();
      TEST_ASSERT_EQUAL_UINT32(before + 1U, fake.callbackCount);
      TEST_ASSERT_EQUAL_INT(static_cast<int>(PendingSurface::NONE),
                            static_cast<int>(g_pendingOperation.surface));
      TEST_ASSERT_FALSE(g_rtc.isJobBusy());
      TEST_ASSERT_FALSE(g_rtc.isEepromBusy());
      // Completion only marks input: the loop still consumes at most 256 bytes.
      const size_t oldRemaining = oldBacklog > cli_shell::MAX_INPUT_BYTES_PER_POLL
          ? oldBacklog - cli_shell::MAX_INPUT_BYTES_PER_POLL : 0U;
      TEST_ASSERT_EQUAL_UINT32(oldRemaining + injectedLength, Serial.available());
      for (uint8_t drain = 0; Serial.available() != 0 && drain < 4U; ++drain) {
        const int bytesBefore = Serial.available();
        loop();
        TEST_ASSERT_LESS_OR_EQUAL_UINT32(cli_shell::MAX_INPUT_BYTES_PER_POLL,
            static_cast<uint32_t>(bytesBefore - Serial.available()));
        TEST_ASSERT_FALSE(g_verbose);
        TEST_ASSERT_EQUAL_UINT32(before + 1U, fake.callbackCount);
        TEST_ASSERT_EQUAL_INT(static_cast<int>(PendingSurface::NONE),
                              static_cast<int>(g_pendingOperation.surface));
      ...
      TEST_ASSERT_EQUAL_INT(0, Serial.available());
      // Terminate the discarded partial line, then accept a fresh complete line.
      Serial.inject("\nverbose 1\n");
      loop();
      TEST_ASSERT_TRUE(g_verbose);
      TEST_ASSERT_EQUAL_INT(0, Serial.available());
...
void test_cli_busy_backlog_is_discarded_after_terminal_poll() {
  FakeRv3032 fake;
  beginCliHarness(fake, false);
  TEST_ASSERT_TRUE(operationAccepted("timer configuration",
      g_rtc.setTimer(50, RV3032::TimerFrequency::Hz1, true)));
  for (uint8_t i = 0; i < 5; ++i) pollPendingOperation(fake.nowMs);
  TEST_ASSERT_TRUE(g_rtc.isOrdinaryJobBusy());
  Serial.inject(std::string(cli_shell::MAX_INPUT_BYTES_PER_POLL, '\n') +
                "verbose 1\nclear_porf");
  loop();
  TEST_ASSERT_FALSE(g_rtc.isOrdinaryJobBusy());
  TEST_ASSERT_GREATER_THAN_INT(0, Serial.available());
  const uint32_t completedCallbacks = fake.callbackCount;
  // This fresh input follows the backlog; its complete line must survive,
  // while both the old complete command and partial tail remain discarded.
  Serial.inject("\nverbose 0\n");
  loop();
  TEST_ASSERT_FALSE(g_verbose);
  TEST_ASSERT_EQUAL_INT(0, Serial.available());
...
int main(int, char**) {
  ...
  RUN_TEST(test_poll_eeprom_zero_budget_tick_and_end_guards);
  ...
  RUN_TEST(test_verified_calendar_rejects_mismatched_readback_weekday);
  ...
  RUN_TEST(test_default_calendar_jobs_admit_without_clock_hook);
  RUN_TEST(test_default_temperature_job_executes_without_clock_hook);
  RUN_TEST(test_impossible_ordinary_states_fail_without_transport);
  RUN_TEST(test_impossible_persistent_states_preserve_cleanup_evidence);
  RUN_TEST(test_password_range_helper_rejects_invalid_spans);
  RUN_TEST(test_default_backup_job_executes_at_large_callback_timeouts);
  ...
  RUN_TEST(test_synchronous_writes_reject_pending_cooperative_work);
  ...
  RUN_TEST(test_fixture_wait_request_log_is_bounded_and_reports_overflow);
  RUN_TEST(test_generic_persistence_uses_full_budget_and_durable_protocol);
  RUN_TEST(test_generic_persistence_honors_extended_busy_poll_window);
  ...
  RUN_TEST(test_polling_predicates_drain_queued_work_behind_ordinary_job);
  ...
  RUN_TEST(test_access_recovery_restores_eerd_after_ready_failure);
  RUN_TEST(test_abandoned_access_cleanup_stays_latched_across_lifecycles);
  ...
  RUN_TEST(test_dispatched_write_one_crossing_cutoff_proves_durability_without_replay);
  RUN_TEST(test_smallest_persistent_budgets_execute);
  RUN_TEST(test_persistent_default_budgets_execute_across_transport_limits);
  RUN_TEST(test_cleanup_busy_phase_expiry_still_restores_access_state);
  RUN_TEST(test_typed_vendor_controls_and_fake_register_semantics);
  RUN_TEST(test_fixture_eeprom_command_and_transfer_evidence);
  ...
  RUN_TEST(test_staged_callback_time_bounds_at_maximum_transfer_timeout);
  ...
  RUN_TEST(test_cli_busy_backlog_is_discarded_after_terminal_poll);
  RUN_TEST(test_cli_terminal_callback_input_is_discarded_on_both_surfaces);
  return UNITY_END();
}
