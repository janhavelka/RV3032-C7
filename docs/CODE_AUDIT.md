...
| F9 | **`examples/common/I2cTransport.h`** — a read that returns no bytes is now mapped to `I2C_NACK_ADDR` instead of `I2C_ERROR`. | On Arduino-ESP32, `endTransmission(false)` performs no bus I/O — it stages the repeated start, and the address phase happens inside `requestFrom()`. So an **absent RTC never produced `I2C_NACK_ADDR`**, and since the driver maps only that code to `DEVICE_NOT_FOUND` and `probe()` is a read, unplugging the RTC made bring-up report `I2C_ERROR` — exactly the case `DEVICE_NOT_FOUND` exists for. This is the file every integrator copies. |
